...
export function renderAnalysis(data, city, marketOverride, { priceUpdateOnly = false } = {}) {
  ...
  const polyBtnHtml = polyUrl
    ? `<a href="${polyUrl}" target="_blank" rel="noopener" class="inline-flex items-center gap-1 border border-[#ff8c00] text-[#ff8c00] text-[9px] font-bold px-[10px] py-1 no-underline whitespace-nowrap tracking-wider transition-all hover:bg-[#ff8c00]/10 hover:shadow-[0_0_8px_rgba(255,140,0,0.25)] shrink-0 cursor-pointer" id="btn-polymarket" title="Open this market on Polymarket to trade">TRADE ON POLYMARKET ↗</a>`
    : '';

  $('edge-grid').innerHTML = `
    <div class="flex flex-col sm:flex-row items-start justify-between gap-[10px]">
      <div class="min-w-0">
        <div class="text-[13px] font-extrabold tracking-[0.02em] ${heroColorCls} break-words" title="Primary trading signal: BUY YES = market is underpriced, FADE = market is overpriced (buy NO), HOLD = no clear edge">${heroAction}</div>
        <div class="text-[10px] text-[#888] mt-[2px] break-words" title="MKT = current Polymarket price, FCST = forecast probability from weather models, EDGE = forecast minus market price">${heroSubtitle}</div>
      </div>
      ${polyBtnHtml}
    </div>
    ...
    ${data.city ? `<div class="text-[8px] text-[#444] mt-1" title="Weather observation station used for this city. Bias correction adjusts forecasts based on historical station-specific errors.">◉ ${data.city.station}${data.stationBias?.reliable ? ` │ BIAS: ${data.stationBias.bias > 0 ? '+' : ''}${data.stationBias.bias.toFixed(2)}°C ${data.stationBias.direction === 'warm' ? '⬆' : data.stationBias.direction === 'cold' ? '⬇' : '●'} (n=${data.stationBias.sampleSize})` : ''}</div>` : ''}
    ${data.liveWeather?.currentTemp != null ? `<div class="text-[10px] text-[#00ff41] font-bold mt-[2px] tracking-wide" title="Current real-time temperature and highest recorded temperature today (so far) from Open-Meteo">LIVE TEMP: ${data.liveWeather.currentTemp.toFixed(1)}°C <span class="text-[#555] mx-1">│</span> TODAY MAX: ${data.liveWeather.maxToday.toFixed(1)}°C</div>` : ''}
  `;
  ...
  if (outcomes.length > 0) {
    $('outcomes-list').innerHTML = `<div class="w-full text-[10px] px-2 pb-[6px] overflow-x-auto">
      <div class="grid grid-cols-[1fr_40px_40px_50px_30px] sm:grid-cols-[1fr_48px_48px_58px_40px] py-[3px] border-b border-[#333] text-[8px] text-[#444] uppercase tracking-[0.1em] font-bold min-w-0">
        <span title="Temperature bracket being traded on Polymarket">OUTCOME</span>
        <span class="text-right" title="Current market YES price on Polymarket (in cents per share)">MKT</span>
        <span class="text-right" title="Forecast probability from ensemble weather models using KDE">FCST</span>
        <span class="text-right" title="Forecast probability minus market price. Positive = underpriced opportunity">EDGE</span>
        <span title="Visual strength of the edge"></span>
      </div>
      ${outcomes.map((o) => {
        ...
        return `<div class="grid grid-cols-[1fr_40px_40px_50px_30px] sm:grid-cols-[1fr_48px_48px_58px_40px] py-[2px] border-b border-[#0a0a0a] items-center transition-colors hover:bg-[#0a0a0a] ${bestBorder} min-w-0" data-token-id="${o.tokenId || ''}" title="${rowTip}">
          <span class="text-[#ccc] truncate text-[10px]">${isBest ? '<span class="text-[#ff8c00] text-[7px]">◆</span> ' : ''}${esc(o.name || o.title)}</span>
          <span class="font-semibold text-right c-muted price-cell" title="Polymarket YES price: ${mkt}¢ per share">${mkt}¢</span>
          <span class="font-semibold text-right c-cyan" title="Weather model ensemble probability: ${fc}%">${fc}%</span>
          <span class="font-semibold text-right edge-cell ${edgeColor(edg)}" title="Edge: forecast (${fc}%) minus market (${mkt}%). ${edgVal > 0 ? 'Positive = underpriced opportunity' : edgVal < 0 ? 'Negative = overpriced, consider fading' : 'No edge'}">${edgVal > 0 ? '+' : ''}${edg}%</span>
          <span class="px-[3px]" title="Edge strength visualization"><span class="block w-full h-[3px] bg-[#111] overflow-hidden"><span class="block h-full ${barColor} transition-[width] duration-300" style="width:${barWidth}%"></span></span></span>
        </div>`;
      }).join('')}
    </div>`;
  }
