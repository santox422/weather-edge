...
export function renderTradingStrategy(data) {
  ...
  html += `<div class="flex items-center justify-between px-2 py-[3px] bg-[#050505] border-b border-[#111]">
    <span class="text-[8px] font-bold text-[#ff8c00] uppercase tracking-[0.15em]" data-tip="Your total trading bankroll in USD. Change this to see position sizes scaled to your capital.">PORTFOLIO</span>
    <span class="text-[11px] text-[#ccc] font-bold" data-tip="Enter your portfolio size in USD">$<input type="number" id="portfolio-input" class="bg-transparent border-b border-[#333] text-[#ff8c00] text-[11px] font-bold w-[60px] outline-none text-right" value="${portfolioSize}" min="100" step="100" /></span>
    <span class="text-[8px] text-[#444]" data-tip="Deploy: % of portfolio to use. Conf: signal confidence. Days: time to resolution.">Deploy ${strat.summary.totalDeployed.toFixed(1)}% │ ${strat.summary.confidence}% conf │ ${strat.summary.daysOut}d</span>
  </div>`;

  if (strat.arbitrage?.isArbitrage) {
    html += `<div class="px-2 py-[3px] text-[9px] text-[#00ff41] bg-[#00ff41]/5 border-b border-[#111] font-bold" data-tip="Complete Set Arbitrage: all YES shares sum to less than $1.00. Buy one of each for a guaranteed profit regardless of outcome.">⚡ ARB — Sum $${strat.arbitrage.sumYesPrices.toFixed(3)} < $1.00 → +${strat.arbitrage.profitIfArb}¢ risk-free</div>`;
  }

  // YES
  if (strat.yesBets.length > 0) {
    html += `<div class="px-2 py-[2px] text-[8px] font-bold c-green uppercase tracking-[0.1em] border-b border-[#111]" data-tip="Brackets where the market is underpricing the probability. Buy YES shares to profit if the temperature lands in this range.">▲ BUY YES — Conviction</div>`;
    ...
    for (const b of strat.yesBets) {
      ...
      const hedge = b.isHedge ? ' <span class="text-[7px] text-[#ff8c00] border border-[#ff8c00] px-[2px] rounded-sm" data-tip="This bet also serves as a hedge for adjacent brackets">H</span>' : '';
  ...
  if ((strat.overpricedNoBets||[]).length > 0) {
    html += `<div class="px-2 py-[2px] text-[8px] font-bold c-purple uppercase tracking-[0.1em] border-b border-[#111]" data-tip="Brackets where the market is overpricing YES. Buy NO shares to profit when the temperature doesn't land here.">▼ FADE — Overpriced (Buy NO)</div>`;
  ...
  if (strat.noBets.length > 0) {
    html += `<div class="px-2 py-[2px] text-[8px] font-bold c-red uppercase tracking-[0.1em] border-b border-[#111]" data-tip="Brackets very unlikely to hit. Sell premium by buying NO shares — high win rate but limited upside.">▼ NO — Safe Premium</div>`;
  ...
  if (strat.longshots.length > 0) {
    html += `<div class="px-2 py-[2px] text-[8px] font-bold c-amber uppercase tracking-[0.1em] border-b border-[#111]" data-tip="Low-probability brackets with massive upside. Small bets with potential for outsized returns.">◆ LONGSHOTS</div>`;
  ...
  html += `<div class="flex h-[4px] mt-[3px] overflow-hidden" data-tip="Visual breakdown of portfolio allocation across bet types">
    <div class="bg-[#00ff41]" style="width:${strat.summary.totalYesPct/total*100}%" data-tip="YES ${strat.summary.totalYesPct}%"></div>
    <div class="bg-[#bb86fc]" style="width:${fPct/total*100}%" data-tip="FADE ${fPct}%"></div>
    <div class="bg-[#ff3333]" style="width:${strat.summary.totalNoPct/total*100}%" data-tip="NO ${strat.summary.totalNoPct}%"></div>
    <div class="bg-[#ff8c00]" style="width:${strat.summary.totalLongshotPct/total*100}%" data-tip="LONG ${strat.summary.totalLongshotPct}%"></div>
  </div>`;
  html += `<div class="flex items-center gap-2 px-2 py-[3px] text-[8px] text-[#666] alloc-legend">
    <span class="c-green" data-tip="Capital allocated to conviction YES bets">■</span> YES ${strat.summary.totalYesPct}%
    ${fPct > 0 ? `<span class="c-purple" data-tip="Capital allocated to fading overpriced brackets">■</span> FADE ${fPct}%` : ''}
    <span class="c-red" data-tip="Capital allocated to safe NO premium bets">■</span> NO ${strat.summary.totalNoPct}%
    <span class="c-amber" data-tip="Capital allocated to low-probability high-reward longshots">■</span> LONG ${strat.summary.totalLongshotPct}%
    <span class="c-muted" data-tip="Undeployed capital held as cash for risk management">■</span> CASH ${(100-strat.summary.totalDeployed).toFixed(1)}%
  </div></div>`;
